...
#iptables -A OUTPUT -o ens3 -p tcp --dport 80 -j ACCEPT
...
iptables -A OUTPUT -s 10.1.2.100 -p tcp --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -d 10.1.2.100 -p tcp --sport 22 -m conntrack --ctstate ESTABLISHED -j ACCEPT
...
iptables -t nat -A POSTROUTING -o eth0 -s 172.2.2.0/24 -j MASQUERADE
iptables -t nat -A POSTROUTING -o eth0 -s 172.2.1.0/24 -j MASQUERADE
