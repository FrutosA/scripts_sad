...
iptables -A FORWARD -s 172.2.2.0/24 -d 172.2.1.2 -j LOG --log-level 6 --log-prefix andres

for PUERTO in "${PUERTOS_DMZ[@]}"; do

  iptables -A FORWARD -s 172.2.2.0/24 -d 172.2.1.2  -o enp0s8 -p tcp --dport $PUERTO -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT

  iptables -A FORWARD -s 172.2.1.2 -d 172.2.2.0/24 -o enp0s9 -p tcp --sport $PUERTO -m conntrack --ctstate ESTABLISHED -j ACCEPT 
...
iptables -A FORWARD -i enp0s8 -s 172.2.1.0/24 -p udp --dport 53 -m conntrack --ctstate  NEW -j ACCEPT
iptables -A FORWARD -o enp0s8 -d 172.2.1.0/24 -p udp --sport 53 -m conntrack --ctstate  ESTABLISHED -j ACCEPT
...
iptables -A INPUT -s 172.2.2.100 -p tcp --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
...
iptables -A FORWARD -i enp0s9 -d 172.2.1.2 -s 172.2.2.100 -p tcp --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
iptables -A FORWARD -i enp0s8 -d 172.2.2.100 -s 172.2.1.2 -p tcp --sport 22 -m conntrack --ctstate ESTABLISHED -j ACCEPT


#Redirigir conexiones ssh de red interna exterior a la DMZ

iptables -A FORWARD -i enp0s9 -d 172.2.0.2 -s 172.2.2.100 -p tcp --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
iptables -A FORWARD -i enp0s3 -d 172.2.2.100 -s 172.2.0.2 -p tcp --sport 22 -m conntrack --ctstate ESTABLISHED -j ACCEPT

# Tráfico DMZ con el exterior para servicios expuestos en DMZ (80)

for PUERTO in "${PUERTOS_DMZ[@]}"; do

  iptables -A FORWARD -d 172.2.1.2 -o enp0s3 -p tcp --dport $PUERTO -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT

  iptables -A FORWARD -s 172.2.1.2 -o enp0s8 -p tcp --sport $PUERTO -m conntrack --ctstate ESTABLISHED -j ACCEPT 
